...
### ### ### ### ### ### ### ### ###  1. LEITURA  E EXPLORAÇÃO DE DADOS ### ### ### ### ### ### ### ### ### 
  ...
  gridExtra::grid.arrange(sp1,sp2,sp3,sp4,sp5,sp6,sp7,sp8,nrow = 4,ncol = 2)
  ...
  # Tipo
  bp3 <- ggplot(db, aes(x = Tipo, y = PMSO)) +
...
### ### ### ### ### ### ### ### ###  2. INICIALIZAÇÃO PARA MODELO ### ### ### ### ### ### ### ### ###
  ...
  summary(modelo)
  #graphics::plot(modelo, lty=0, pch=19, col="blue")
  ...
  linear1log <- ggplot(data = db, aes(x = PMSO, y = yfit1)) +
  ...
  ## Cross- validarion | Leave-one-out <> Linear regression
  ...
  ggplot(data = db, aes(x = PMSO, y = yfit1_linear)) +
    ...
    labs(x = "Observado", y = "Estimatedo") +
    ggtitle("PMSO: estimado x observado",
            subtitle = "Modelo de regressão leave-one-out - escala Log")+
    theme_bw()+
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5),
...
### ### ### ###  MODELO GAMA  - Implementacao Prof. Marcelo (02/12/2022)
  ...
  modelo_gm <- MASS::stepAIC(modelo_gm, trace = TRUE)


  #write.xlsx(summary(modelo_gm),"~/Library/CloudStorage/OneDrive-Pessoal/Documentos/7. Especialização/3. TCC/8. Resultados modelos Gama/Resultados.xlsx", sheetName = "Modelo1", 
  #           colNames = TRUE, rowNames = TRUE, append = FALSE)

  #p-value for deviance
  1 - pchisq(modelo_gm$deviance, modelo_gm$df.residual)

  yfit_gama= predict(modelo_gm,newdata = db)

  yr2_gama  <- db$PMSO
  SQT_gama   <- sum( (yr2_gama - mean(yr2_gama))^2 )
  SQres_gama <- sum( (yr2_gama - yfit_gama)^2 )
  R2_gama = (1 - SQres_gama/SQT_gama)
  R2_gama

  y_log_gama     <- log(db$PMSO)
  SQT_log_gama   <- sum( (y_log_gama - mean(y_log_gama))^2 )
  SQres_log_gama <- sum( (y_log_gama - log(yfit_gama))^2 )
  R2_log_gama = (1 - SQres_log_gama/SQT_log_gama)
  R2_log_gama


  ## Modelo (1)
  mg1 <- ggplot(data = db, aes(x = PMSO, y = yfit_gama)) + 
    ...
    xlab("Observado") + ylab("Estimado") +
    ggtitle("PMSO: estimado x observado",
            subtitle = "Modelo de regressão Gama (1)")+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          plot.subtitle = element_text(hjust = 0.5),
          axis.text = element_text(face = "bold", size = 8),
  ...
  mg1_log <- ggplot(data = db, aes(x = PMSO, y = yfit_gama)) +
    ...
    labs(x = "Observado", y = "Estimatedo") +
    ggtitle("PMSO estimado x observado",
            subtitle = "Modelo de regressão Gama (1) - escala Log")+
    theme_bw()+
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5),
  ...
  ### CROSS VALIDATION | LEAVE-ONE-OUT <> GAMA REGRESSION
  ...
  plot(modelo_gm2)
  summary(modelo_gm2)
  1 - pchisq(modelo_gm2$deviance, modelo_gm2$df.residual)

  #modelo_gm$coefficients
  #modelo_gm2$coefficients
  ...
  mg2 <-ggplot(data = db, aes(x = PMSO, y = yfit)) + 
    geom_point(color = "blue", size = 3, shape = 19) +
    scale_y_continuous(labels = label_reais, limits = c(0, 2000000))+
    scale_x_continuous(labels = label_reais, limits = c(0, 2000000))+
    geom_abline(intercept = 0, slope = 1, color = "red", size = 1,linetype = "dotted") +
    xlab("Observado") + ylab("Estimado") +
    ggtitle("PMSO: estimado x observado",
            subtitle = "Modelo de regressão Gama (2)")+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          plot.subtitle = element_text(hjust = 0.5),
          axis.text = element_text(face = "bold", size = 8),
  ...
  gridExtra::grid.arrange(mg1,mg2,ncol = 2)
  ...
  ggplot(data = db, aes(x = PMSO, y = yfit)) +
    ...
    labs(x = "Observado", y = "Estimatedo") +
    ggtitle("PMSO estimado x observado",
            subtitle = "Modelo de regressão Gama (2) - escala Log")+
    theme_bw()+
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5),
...
ggplot(data = db_r, aes(x = PMSO, y = pmso_predict)) + 
  geom_point(color = "blue", size = 3, shape = 19) +
  scale_y_continuous(labels = label_reais, limits = c(-200000, 1800000))+
  scale_x_continuous(labels = label_reais, limits = c(-200000, 1800000))+
  geom_abline(intercept = 0, slope = 1, color = "red", size = 1,linetype = "dotted") +
  xlab("Observado") + ylab("Estimado") +
  ggtitle("PMSO estimado x observado",
          subtitle = "Modelo de programação linear - completo")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        plot.subtitle = element_text(hjust = 0.5),
        axis.text = element_text(face = "bold", size = 8),
...
ggplot(data = db_r, aes(x = PMSO,  y = pmso_predict)) +
  ...
  labs(x = "Observado", y = "Estimatedo") +
  ggtitle("PMSO estimado x observado",
          subtitle = "Modelo linear - escala Log")+
  theme_bw()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5),
...
ggplot(data = TABULAR, aes(x = PMSO, y = pmso_val)) + 
  ...
  xlab("Observado") + ylab("Estimado") +
  ggtitle("PMSO estimado x observado",
          subtitle = "Modelo linear - Leave-one-out")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        plot.subtitle = element_text(hjust = 0.5),
        axis.text = element_text(face = "bold", size = 8),
...
ggplot(data = TABULAR, aes(x = PMSO, y = pmso_val)) +
  ...
  labs(x = "Observado", y = "Estimatedo") +
  ggtitle("PMSO estimado x observado",
          subtitle = "Modelo linear - Leave-one-out - escala Log")+
  theme_bw()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5),
...
bh0 = ggplot(res_boot, aes(x = alpha)) + 
  geom_histogram(bins = 50, fill = "grey", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 0"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh1 = ggplot(res_boot, aes(x = beta1)) + 
  geom_histogram(bins = 50, fill = "darkblue", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 1"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh2 = ggplot(res_boot, aes(x = beta2)) + 
  geom_histogram(bins = 50, fill = "red", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 2"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh3 = ggplot(res_boot, aes(x = beta3)) + 
  geom_histogram(bins = 50, fill = "darkgreen", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 3"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh4 = ggplot(res_boot, aes(x = beta4)) + 
  geom_histogram(bins = 50, fill = "navy", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 4"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh5 = ggplot(res_boot, aes(x = beta5)) + 
  geom_histogram(bins = 50, fill = "maroon", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 5"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh6 = ggplot(res_boot, aes(x = beta6)) + 
  geom_histogram(bins = 50, fill = "orange", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 6"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh7 = ggplot(res_boot, aes(x = beta7)) + 
  geom_histogram(bins = 50, fill = "purple", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 7"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
bh8 = ggplot(res_boot, aes(x = beta8)) + 
  geom_histogram(bins = 50, fill = "brown", color = "white") +
  scale_x_continuous(labels = label_dots, name = "") +
  scale_y_continuous(name = "Freq.")+
  ggtitle(expression(paste(bold("Beta 8"))),
          subtitle = "Bootstrap")+
  theme_classic()+
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
# packHV::hist_boxplot(res_boot$alpha,main="Histograma - Método Bootstrap", 
#                      col="grey",
#                      xlab="Beta 0")

# packHV::hist_boxplot(res_boot$beta1,main="Histograma - Método Bootstrap", 
#                      col="darkblue",
#                      xlab="Beta 1")

# packHV::hist_boxplot(res_boot$beta2,main="Histograma - Método Bootstrap", 
#                      col="red",
#                      xlab="Beta 2")
# 
# packHV::hist_boxplot(res_boot$beta3,main="Histograma - Método Bootstrap", 
#                      col="darkgreen",
#                      xlab="Beta 3")

# packHV::hist_boxplot(res_boot$beta4,main="Histograma - Método Bootstrap", 
#                      col="navy",
#                      xlab="Beta 4")
# 
# packHV::hist_boxplot(res_boot$beta5,main="Histograma - Método Bootstrap", 
#                      col="maroon",
#                      xlab="Beta 5")

# packHV::hist_boxplot(res_boot$beta6,main="Histograma - Método Bootstrap", 
#                      col="orange",
#                      xlab="Beta 6")

# packHV::hist_boxplot(res_boot$beta7,main="Histograma - Método Bootstrap", 
#                      col="purple",
#                      xlab="Beta 7")

# packHV::hist_boxplot(res_boot$beta8,main="Histograma - Método Bootstrap", 
#                      col="brown",
#                      xlab="Beta 8")
#                      

gridExtra::grid.arrange(bh0)
gridExtra::grid.arrange(bh1,bh2,bh3,bh4,nrow = 2, ncol = 2)
gridExtra::grid.arrange(bh5,bh6,bh7,bh8,nrow = 2 ,ncol = 2)   
