  ...
  ## Ano
  db2 = with(dados,dplyr::select(dados, Concessionaria, Tipo,
                                Ano, PMSO, rede.menor.230,
                                rede.maior.230,MVA,Mvar,
                                modulos.sub.menor230,modulos.sub.maior230,
                                modulos.manobra.menor230,modulos.manobra.maior230))
  ...
  modelo_mass2 = masslm(db2[,3:12], "PMSO")
  modelo_mass2 =  as.data.frame(modelo_mass2)
  modelo_mass2 = modelo_mass2[order(modelo_mass2$R.squared,decreasing = TRUE),]
  print(modelo_mass2)
  ...
  ### Histograma
  with(db_df,packHV::hist_boxplot(PMSO, main="Histograma", 
                       col="light blue",
                       xlab="PMSO"
                       ))
  ...
  ggplot(db, aes(x = PMSO)) + 
    ...
    scale_x_continuous(labels = comma, name = "PMSO") +
  ...
  ggplot(db, aes(X1, PMSO)) + 
    geom_point(pch = 20, color = "darkblue") + 
    ...
    ggplot(db, aes(X3, PMSO)) + 
      geom_point(pch = 20, color = "darkgreen") + 
    ...
    ggplot(db, aes(X5, PMSO)) + 
      geom_point(pch = 20, color = "maroon") + 
